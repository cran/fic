...
test_that("error handling", {
  ...
  expect_error(
    fic(wide.glm, inds=inds1, inds0=inds0, focus=focus_plogis, X=X, 
        sub=list(mod1.glm, mod2.glm))
    , "not found")
  mod2.glm <- glm(low ~ lwtkg + age, data=birthwt, family=binomial)
  expect_error(fic(wide.glm, inds=inds1, inds0=inds0, focus=focus_plogis, 
                   X=X, sub=list(mod1.glm, mod2.glm)), "`sub` of length")
